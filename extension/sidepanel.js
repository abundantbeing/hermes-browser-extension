...
function restoreBackendQueuedSteerDraft(text) {
  ...
  // Steer wurde nicht injiziert — laufenden Versuch verwerfen (Test-Contract tests/common.test.mjs)
  pendingSteerText = '';
...
async function sendSteerText(text) {
  ...
  // Versuch tracken, bis der Stream bestätigt oder der queued-Fallback greift (Test-Contract)
...
async function steerCurrentDraft() {
  ...
  try {
    ...
    // Versuch fehlgeschlagen — Tracking zurücksetzen (Test-Contract)
    pendingSteerText = '';
...
async function askHermes(userText, turnAttachments = [...attachments], turnOptions = {}) {
  ...
  try {
    ...
    activeRunId = '';
    // Stream-Ende — laufendes Steer-Tracking aufräumen (Test-Contract)
    pendingSteerText = '';
    sending = false;
    updateComposerBusyState();
    renderContextWindow();
